    ...
    ui(new Ui::MainWindow)
{
    ...
    Eigen::Vector3f vector1(1, 0, 1);
    vector1 << 1, 2, 3;
    float a = vector1(0);
    vector1(1) = a;
    std::cout << "vector1 :" << vector1.transpose() << std::endl;

    /*Eigen::VectorXf vectorx;
    vectorx << 1, 2, 3, 4, 5;
    std::cout << "vectorx :" << vectorx.transpose() << std::endl;*/

    Eigen::Matrix3f mat3;
    mat3 << 1, 2, 3, 4, 5, 6, 7, 8, 9;
    std::cout << "mat3 :" << mat3.transpose() << std::endl;
    ...
    aff = trans * anax;     // != anax * trans
    std::cout << "transform \n" << aff.matrix() << std::endl;

    aff = aff * aff;        // == aff.matrix() * aff.matrix()
    std::cout << "transform2 \n" << aff.matrix() << std::endl;
